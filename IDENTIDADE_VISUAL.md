...
- **Várias fotos lado a lado** (2 a 8): mesma ALTURA fixa para todas, LARGURA proporcional à razão de aspecto real de cada uma, centralizadas como grupo com um gap fixo entre elas.
  ...
  - **Com 5+ fotos pequenas, o gargalo real costuma ser a LEGENDA (texto), não a foto** — texto de legenda geralmente é mais largo que uma foto-retrato estreita. Calcular o espaço pela legenda mais longa, não só pela proporção da imagem. Alternativa preferida a encurtar o nome: **quebrar a legenda em 2 linhas** (dividir no meio das palavras) quando o comprimento estimado do texto (`len(legenda) * ~0.5 * font-size`) exceder a largura da coluna da foto + gap — mantém o nome completo sem sobrepor a legenda vizinha (usado em `genogramafilhosfelipeanalia.svg`, 5 fotos estreitas lado a lado). Só encurtar pro primeiro nome se mesmo com 2 linhas não couber, e avisar o usuário dessa escolha.
...
**CORRIGIDO (2026-08-29): margem negativa pode gerar espaçamento negativo.** Quando a altura-alvo é pequena (poucas linhas + fotos que não cabem confortavelmente), a margem calculada no passo 1 pode ser **negativa** (o conteúdo base já é maior que o alvo). Nesse caso, a fórmula de redistribuição (que multiplica a margem por uma fração pra cada gap) pode fazer `row_gap` ou `gap_row_photo` ficarem **negativos**, causando sobreposição visual de linhas/fotos. Sempre aplicar um **clamp mínimo** em cada valor de espaçamento (ex. `row_gap = max(row_gap, 0.8)`, `gap_row_photo = max(gap_row_photo, 2.0)`) depois de aplicar a fórmula, e se ainda sobrar overshoot depois do clamp, reduzir a altura BASE das fotos (aumentar o valor subtraído da largura disponível na fórmula de `PHOTO_H`) antes de mexer nos espaçamentos.
...
6. **(CORRIGIDO)** Quando uma seção é uma lista corrida homogênea sem subgrupos nomeados, pertencente toda ao mesmo casal-tronco (ex: "Bisnetos de X e Y", "Trinetos de X e Y" — todos descendentes dos mesmos bisavós), o **título da seção aparece só UMA VEZ**, no início, e a lista flui livremente pelas partes seguintes **sem repetir o título nem qualquer cabeçalho** nas continuações — nem mesmo o nome real da seção de novo. Não se usa mais chunk fixo (15-16) com cabeçalho repetido; em vez disso, calcula-se dinamicamente quantas linhas cabem no espaço restante de cada página (`avail // row_unit`) e a lista é fatiada nesse ponto, prosseguindo na parte seguinte sem título algum. Isso permite aproveitar 100% da altura vertical disponível em cada parte, já que não se gasta espaço com cabeçalhos repetidos. (Ver script de referência atualizado na seção 8 — função `flow_list`/`emit_flow`.)
...
**CORRIGIDO (2026-08-28): empacotamento guloso (greedy) cria partes muito desiguais.** O algoritmo guloso simples (empilhar o máximo possível em cada página antes de virar página) é ótimo pra minimizar o número de páginas, mas pode gerar uma distribuição muito desigual entre elas — uma parte com só 20% do conteúdo preenchido e a próxima com 95%, porque um grupo grande "quase coube" e sobrou uma folga enorme. Nesse caso, mesmo redistribuindo a folga nos espaçamentos (técnica da seção 5), o resultado ainda parece errado porque o fator de esticamento necessário fica absurdo (7-8x), criando espaços em branco grandes e desproporcionais entre mini-grupos.

**Técnica correta quando isso acontece**: em vez de aceitar a divisão gulosa, **rebalancear manualmente os pontos de corte** entre as partes (sempre em limite de grupo inteiro, nunca dividindo um grupo) para que cada parte fique com uma quantidade de conteúdo bruto (antes de esticar) o mais parecida possível entre si — mesmo que isso signifique deixar uma parte "artificialmente" mais vazia no bruto do que o guloso faria (ex: colocar só 1 bloco grande sozinho numa parte, em vez de tentar espremer mais um bloco pequeno). Depois de rebalancear os cortes, aplicar o esticamento de espaçamento (linha a linha, não só nos gaps entre grupos) por página — o fator de esticamento fica parecido entre as páginas, dando uma aparência visual uniforme e sem grandes vazios ao longo de toda a série. Ver script de referência (`gen_alderinafirmo_v2.py`, série `genogramafilhosnetosalderinafirmo`).
