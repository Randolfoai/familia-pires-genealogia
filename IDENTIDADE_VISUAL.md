...
- **Fotos "estilo documento" (3x4, retrato tipo carteira)**: quando o usuário pedir esse enquadramento explicitamente (ex: "evitar corpo inteiro"), usar uma caixa de proporção FIXA 3:4 (largura:altura = 0,75) igual pra todas as fotos, em vez de preservar a proporção nativa de cada imagem. Calcular `scale = largura_da_caixa / largura_da_imagem` (a imagem preenche a largura inteira) e deixar a altura da imagem escalada exceder a altura da caixa livremente — o `clipPath` já corta o excesso embaixo automaticamente, e como a imagem é colocada com o topo alinhado ao topo da caixa (sem centralizar verticalmente), o corte natural remove o corpo/pernas e mantém rosto+ombros visíveis. Não precisa inspecionar cada foto individualmente pra decidir o corte — esse "ancorar no topo" funciona bem pra fotos de retrato vertical (corpo inteiro ou busto).
